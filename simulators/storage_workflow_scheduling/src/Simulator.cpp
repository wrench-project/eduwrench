 ...
 * @description
 */
void generateWorkflow(wrench::Workflow *workflow, int num_inputs, int num_blue, int num_yellow,
        int num_orange, int num_red) {
    // Blue task
    auto blue_task = workflow->addTask("blue_task", 1000000, num_blue, num_blue, 1, 10000);
    auto yellow_task = workflow->addTask("yellow_task", 1000, num_yellow, num_yellow, 1, 10000);
    auto orange_task = workflow->addTask("orange_task", 1000, num_orange, num_orange, 1, 10000);
    auto red_task  = workflow->addTask("red_task", 1000000, num_red,  num_red, 1.0, 10000);
...
int main(int argc, char **argv) {
    ...
    int NUM_INPUTS;
    int NUM_BLUE_CORES;
    int NUM_YELLOW_CORES;
    int NUM_ORANGE_CORES;
    int NUM_RED_CORES;
    ...
    try {
        ...
        NUM_INPUTS = std::stoi(std::string(argv[1]));
        NUM_BLUE_CORES = std::stoi(std::string(argv[2]));
        NUM_YELLOW_CORES = std::stoi(std::string(argv[3]));
        NUM_ORANGE_CORES = std::stoi(std::string(argv[4]));
        NUM_RED_CORES = std::stoi(std::string(argv[5]));
    } catch (std::invalid_argument &e) {
        std::cerr << e.what() << std::endl;
        std::cerr << "Usage: " << argv[0]
                  << "<NUM_INPUTS> <NUM_BLUE_CORES> <NUM_YELLOW_CORES> <NUM_ORANGE_CORES> <NUM_RED_CORES>"
                  << std::endl;
        return 1;
    }
    ...
    generateWorkflow(&workflow, NUM_INPUTS, NUM_BLUE_CORES, NUM_YELLOW_CORES, NUM_ORANGE_CORES, NUM_RED_CORES);
