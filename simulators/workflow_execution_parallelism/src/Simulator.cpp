...
void generateTaskJoinWorkflow(
    ...
    // add final task
    auto final_task = workflow->addTask("task" + std::to_string(num_tasks_to_join), (FLOPS / 12), MIN_CORES, MAX_CORES,
                                        PARALLEL_EFFICIENCY,
    ...
    for (int i = 0; i < num_tasks_to_join; ++i) {
        ...
        auto current_task = workflow->addTask("task" + std::to_string(i), FLOPS, MIN_CORES, MAX_CORES,
                                              PARALLEL_EFFICIENCY, MEMORY_REQUIREMENT + (2.0 * file_size));
...
int main(int argc, char **argv) {
    ...
    try {
        ...
        if (NUM_HPC_NODES < 1 || NUM_HPC_NODES > MAX_NODES) {
            std::cerr << "Invalid number of HPC execution nodes. Enter a value in the range [1, " +
                         std::to_string(MAX_NODES) + "]" << std::endl;
        ...
        if (NUM_CORES_PER_HPC_NODE < 1 || NUM_CORES_PER_HPC_NODE > MAX_CORES) {
            std::cerr << "Invalid number of cores per HPC node. Enter a value in the range [1, " +
                         std::to_string(MAX_CORES) + "]" << std::endl;
        ...
        if (NUM_TASKS_TO_JOIN < 2 || NUM_TASKS_TO_JOIN > MAX_TASKS_TO_JOIN) {
            std::cerr << "Invalid number of tasks to join. Enter a value in the range [2, " +
                         std::to_string(MAX_TASKS_TO_JOIN) + "]" << std::endl;
        ...
        if (FILE_SIZE < 1.0 || FILE_SIZE > MAX_FILE_SIZE) {
            std::cerr << "Invalid file sizes. Enter a value in the range [1, " + std::to_string(MAX_FILE_SIZE) + "]"
                      << std::endl;
        ...
        std::cerr
                << "Usage: activity_2_simulator <num_hpc_nodes> <num_hpc_cores_per_node> <num_tasks_to_join> <file_size> <task_memory_requirement>"
                << std::endl;
        std::cerr << "   num_hpc_nodes: number of execution nodes in the range [1, " + std::to_string(MAX_NODES) + "]"
                  << std::endl;
        std::cerr
                << "   num_hpc_cores_per_node: number of cores per node in the range [1, " + std::to_string(MAX_CORES) +
                   "]" << std::endl;
        std::cerr << "   num_tasks_to_join: the number of independent tasks to join in the range [2, " +
                     std::to_string(MAX_TASKS_TO_JOIN) + "]" << std::endl;
        std::cerr << "   file_size: number of bytes in the range [1, " + std::to_string(MAX_FILE_SIZE) + "]"
                  << std::endl;
    ...
    std::sort(hpc_nodes.begin(), hpc_nodes.end(),
              [&index_of_node_number](const simgrid::s4u::Host *lhs, const simgrid::s4u::Host *rhs) {
                  return std::stoi(lhs->get_name().substr(index_of_node_number)) <
    ...
    for (auto node = hpc_nodes.begin() + 1; node != hpc_nodes.end(); ++node) {
        compute_nodes.push_back((*node)->get_name());
    }

    auto compute_service = simulation.add(new wrench::BareMetalComputeService(COMPUTE_HOST, compute_nodes, {}, {}));
    ...
    simulation.getOutput().dumpWorkflowExecutionJSON(&workflow, "/tmp/workflow_data.json", true);
