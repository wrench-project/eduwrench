...
namespace wrench {
    ...
    int ActivityWMS::main() {
        ...
        while (true) {
            ...
            // Get the available storage services, in this case only one
            const std::set<std::shared_ptr<StorageService>> storage_services = this->getAvailableStorageServices();
     ...
     * @param event
     */
    void ActivityWMS::processEventStandardJobFailure(std::shared_ptr<StandardJobFailedEvent> event) {
        auto standard_job = event->standard_job;
        TerminalOutput::setThisProcessLoggingColor(TerminalOutput::Color::COLOR_RED);
