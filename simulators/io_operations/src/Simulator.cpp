...
int main(int argc, char **argv) {
    ...
    try {
        ...
        std::cerr << "Usage: " << argv[0] << " <task_read> <task_write> <task_num> <task_gflop> <io_overlap>"
                  << std::endl;
        ...
        std::cerr << "   io_overlap: whether read/write can be done concurrently with computation [true|false]"
                  << std::endl;
        std::cerr << "" << std::endl;
    ...
    simulation.getOutput().dumpUnifiedJSON(&workflow, "/tmp/workflow_data.json", true, true, true, false, false);
