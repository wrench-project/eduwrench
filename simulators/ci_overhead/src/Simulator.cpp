...
void generateWorkflow(wrench::Workflow *workflow,
    ...
    single_task = workflow->addTask("slow_server_task", task_work_in_gf * GFLOP, MIN_CORES, MAX_CORES, 8 * GB);
...
void generatePlatform(const std::string &platform_file_path,
                      int link_1_latency,
                      int link_1_bandwidth,
                      int link_2_bandwidth,
                      int disk_toggle, int disk_speed) {
...
int main(int argc, char **argv) {
    ...
    try {
        ...
        COMPUTE_1_OVERHEAD = std::string(argv[9]);
        ...
        std::cerr << "Usage: " << argv[0]
                  << " <server_1_link_latency> <server_1_link_speed> <server_2_link_speed> <buffer> <host_select> <disk select> <disk speed> <file size> <server_1_overhead> <server_2_overhead> <task_work>"
                  << std::endl;
    ...
    generatePlatform(platform_file_path, SERVER_1_LINK_LATENCY, SERVER_1_LINK_BANDWIDTH, SERVER_2_LINK_BANDWIDTH,
                     DISK_TOGGLE, DISK_SPEED);
    ...
    if (HOST_SELECT == 1) {
        compute_service = simulation.add(
                new wrench::BareMetalComputeService(
                        CLIENT,
                        {{SERVER1, std::make_tuple(wrench::ComputeService::ALL_CORES,
                                                   wrench::ComputeService::ALL_RAM)}},
                        "",
                        {
                                {wrench::BareMetalComputeServiceProperty::TASK_STARTUP_OVERHEAD, COMPUTE_1_OVERHEAD},
                        },
                        {}
                )
        );
        ...
        compute_service = simulation.add(
                new wrench::BareMetalComputeService(
                        CLIENT,
                        {{SERVER2, std::make_tuple(wrench::ComputeService::ALL_CORES,
                                                   wrench::ComputeService::ALL_RAM)}},
                        "",
                        {
                                {wrench::BareMetalComputeServiceProperty::TASK_STARTUP_OVERHEAD, COMPUTE_2_OVERHEAD},
                        },
                        {}
                )
        );
