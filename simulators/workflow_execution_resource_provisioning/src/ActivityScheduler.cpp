    ...
    void ActivityScheduler::scheduleTasks(const std::set<std::shared_ptr<ComputeService>> &compute_services,
        ...
        for (const auto &task : tasks_to_submit) {
            ...
            for (const auto &file : task->getInputFiles()) {
                if (taskHasChildren) {
                    file_locations.insert(std::make_pair(file, FileLocation::LOCATION(storage_services.at("infrastructure.org/storage"))));
                } else {
                    file_locations.insert(std::make_pair(file, FileLocation::LOCATION(storage_services.at("infrastructure.org/compute"))));
                }
            }
