...
const About = () => {
  ...
  useEffect(() => {
    axios.post("http://" + window.location.hostname + ":" + process.env.GATSBY_BACKEND_PORT + "/get/usage_statistics").then(
      response => {
  ...
  useEffect(() => {
    axios.post("http://" + window.location.hostname + ":" + process.env.GATSBY_BACKEND_PORT + "/get/global_statistics").then(
      response => {
        console.log('================',response.data)
