...
- Understand the impact of I/O on parallel performance
...
module]({{site.baseurl}}/pedagogic_modules/single_core_computing), 
the execution proceeds as depicted in the figure below:
...
17 second (only the first file read is not hidden by I/O). This is a great
...
We execute this program on a compute with  cores that compute at 
...
Note that there are other options to run this program. For instance,
...
disk bandwidth sharing, Task #2 starts at time 2 and Task #1 starts at time 3. So here also, the execution takes 9s. 
